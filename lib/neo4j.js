            ...
            if (item.hasOwnProperty(property)) {
                ...
                if(property.indexOf("Id", property.length - 2) !== -1){
                    ...
                    if(item[property].reverse){
                        ...
                        newfilter += (newfilter.length>0 ? " AND " : " where ") + " n.id in " + (item[property].inq ? JSON.stringify(item[property].inq) : "[" + JSON.stringify(item[property]) + "]")  ;
                        endquery = " ON MATCH SET o." + property + " = n.id return distinct(o) as n";

                    }else{
                        // Proper way
                        subquery += 'with n MATCH (n)-[r:HAS_' + subModel.toUpperCase() + ']->(o:' + subModel + ') where o.id in ' + (item[property].inq ? JSON.stringify(item[property].inq) : "[" + JSON.stringify(item[property]) + "]");
                        endquery = " return distinct(n) as n";
                    }
                }else{
                    // Regular property
                    if(!item[property].reverse){
                        newfilter += (newfilter.length>0 ? " AND " : "where ") + (item[property].inq ? 'n.' + property + ' IN ' + JSON.stringify(item[property].inq) : 'n.' + property + ' = ' + JSON.stringify(item[property])) ;
                    }
                }
            }
        }
