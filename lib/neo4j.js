...
neo4j.all = function (model, filter, cb) {
        ...
        for (var property in item) {
            if (item.hasOwnProperty(property)) {
                ...
                if(property.indexOf("Id", property.length - 2) !== -1){
                    // This is a sub query
                    var subModel = property.replace("Id","");
                    ...
                    if(item[property].reverse){
                        // Reversed lookup
                        startquery = "match (n:" + subModel + ") %filter% "
                        subquery += 'with n MERGE (n)-[r:HAS_' + model.toUpperCase() + ']->(o:' + model + ')';
                        newfilter += (newfilter.length>0 ? " AND " : " where ") + " " + getInQuery("n","id",(item[property].inq ? item[property].inq : JSON.stringify(item[property]);
                        endquery = " ON MATCH SET o." + property + " = n.id return distinct(o) as n";

                    }else{
                        // Proper way
                        subquery += 'with n MATCH (n)-[r:HAS_' + subModel.toUpperCase() + ']->(o:' + subModel + ') where ' + getInQuery('o','id', (item[property].inq ? item[property].inq : item[property]));
                        endquery = " return distinct(n) as n";
                    }
                }else{
                    // Regular property
                    if(!item[property].reverse){
                        newfilter += (newfilter.length>0 ? " AND " : "where ") + getInQuery('n',property,(item[property].inq ? item[property].inq : item[property])) ;
                    }
                }
            }
        }
    ...
    // console.log('got something ' + newfilter);
    var query = (startquery + subquery + endquery)
        .replace('%node%', model)
...
function getInQuery(letter, propertyName,value){
    return 'has(' + letter + '.' + propertyName + ') and any(' + letter + ' in ' + propertyName + ' where ' + letter + ' in ' + (value.constructor === Array? JSON.stringify(value) : "[" + value + "]");
}
