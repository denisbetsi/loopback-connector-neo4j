...
neo4j.all = function (model, filter, cb) {
                ...
                if(property.toUpperCase()=='OFFSET'){
                    ...
                    if(property.indexOf("Id", property.length - 2) !== -1){
                        ...
                        if(item[property].reverse){
                            ...
                            subquery += ' with n match (oo:' + model + ') %filter% with n,oo MATCH (n)-[r:HAS_' + model.toUpperCase() + ']->(oo) ';
                            ...
                            // Proper way
                            subquery += 'with n MATCH (n)-[r:HAS_' + subModel.toUpperCase() + ']->(oo:' + subModel + ') where ' + getInQuery('oo.id', (item[property].inq ? item[property].inq : item[property]));
    ...
    var query = (startquery + countSubQuery + subquery + endquery + ",total ");
        .replace('%node%', model)
