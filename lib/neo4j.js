...
neo4j.all = function (model, filter, cb) {
                ...
                if(property.toUpperCase()=='OFFSET'){
                    ...
                    if(property.indexOf("Id", property.length - 2) !== -1){
                        ...
                        if(item[property].reverse){
                            ...
                            // Proper way
                            subquery += ' MATCH (n:' + model + ')-[r:HAS_' + subModel.toUpperCase() + ']->(oo:' + subModel + ') where ' + getInQuery('oo.id', (item[property].inq ? item[property].inq : item[property]));
    ...
    var countSubQuery = " " + subquery.split("oo").join("ooo") + " with count(*) as total ";
    ...
    this.db.query(query, params, function (err, results) {
        ...
        cb(null, results.map(function (r) {
            var ret = r.n.data;
            // Append total if there
            if(r.total){
                // Adding total
                console.log("ADDING TOTAL : ",r.total);
                ret['__totalRecords'] = r.total;
            }
...
function getInQuery(propertyName,value){
    return 'exists(' + propertyName + ') and any(bbKing in ' + propertyName + ' where bbKing in ' + (value.constructor === Array? JSON.stringify(value) : "[" + JSON.stringify(value) + "]") + ")";
        // return '';
}
